...
| 本地验收状态 | 通过 |
| 远端发布验收状态 | 通过 |
| 上层系统解锁状态 | Topic 1 已解锁；其余上层仍锁定 |
| 冻结原则 | Phase 1.1 仅允许向后兼容扩展，不允许修改既有安全与事务语义 |

本报告区分“本地工程验收”和“远端发布验收”。本地工程验收证明代码、数据库、
容器、契约、供应链清单和故障恢复能力成立；GitHub 托管 Linux 环境对同一提交执行
完整 `Release quality redline`，`main` 经过 API 回读确认进入强保护状态。因此
Phase 1.1 正式冻结，仅解锁 Topic 1 repository/service 与知识拓扑业务实现。
...
4. `main` 要求 `Release quality redline`、PR、Code Owner 审核、至少一个批准、
   conversation resolution 和线性历史；管理员同样受约束。
5. force-push、删除和匹配保护引用的新建均被禁止；Actions 默认 token 为只读且不能
   批准 PR。

精确 run URL、提交和保护状态同时记录于 `acceptance-status.json`、GitHub Actions
以及本地忽略目录 `artifacts/quality-gates/` 的机器证据中。
