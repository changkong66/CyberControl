...
| 本地验收状态 | 通过 |
| 远端发布验收状态 | 通过 |
| 上层系统解锁状态 | Topic1 已解锁，其余上层保持锁定 |
| 冻结原则 | Phase1.1 只允许向后兼容扩展，禁止侵入式修改 |

本报告区分“本地工程验收”和“远端发布验收”。本地工程验收证明代码、数据库、
容器、契约、供应链清单和故障恢复能力成立；GitHub 托管 Linux 环境对同一提交执行
完整 `Release quality redline`。仓库切换为 Public 后，Classic Branch Protection 与
Repository Rulesets 已通过 API 配置和回读；实际 Git 协议探针证明直接推送、强推、
删除主干和删除受保护标签均被阻断。公开仓库全历史密钥扫描、GitHub Secret Scanning
和 Push Protection 同步通过。因此 Phase1.1 状态为 `ACCEPTED`，Topic1 正式解锁。
...
| 8. CI/CD 门禁 | 本地与远端通过 | Actions、SBOM、许可证、审计、Ruff、Go、Vue、容器、Gitleaks、Trivy |
...
1. Repository：`changkong66/CyberControl`，Public 仓库身份和 admin 权限已验证。
2. Phase 分支治理提交 `39e7f016a071f435ad9fcf56876ae5866625ef97` 的远端 run
   `29399037874` 成功，最终 job `87299312349` 为 success。
3. `main` 初始化 run `29399212865` 成功，最终 job `87299892896` 为 success。
4. 最终验收提交 `fe3482bb8662281c21359af66e952e69e827b958` 在 Phase 分支 run
   `29399883914` 和 main run `29400001506` 均通过完整红线。
5. 默认分支已设为 `main`，merge commit 已禁用，仅允许 squash/rebase，合并后自动
   删除分支；Actions 默认 token 为只读且不能批准 PR。
6. Classic Protection API 回读通过；`main-release-governance` 与
   `immutable-release-tags` 两个 Ruleset 均为 Active。
7. 直接 push、force-push、删除 `main` 和删除受保护标签的探针全部被 GitHub 拒绝，
   远端 SHA 未变化。
8. Gitleaks 历史/工作树与 GitHub open secret alerts 均为 0；Secret Scanning 和 Push
   Protection 已启用。

精确 run URL、提交、成功门禁和服务端保护证据同时记录于 `acceptance-status.json`、GitHub
Actions 以及本地忽略目录 `artifacts/quality-gates/` 的机器证据中。
...
6. `main` 分支保护和 Repository Rulesets 经 API 回读确认；
7. 远端 run URL、commit SHA、SBOM 和测试证据写入验收台账。

后续解锁顺序固定为：

1. Topic1 冻结数据模型的 repository/service 绑定，当前已解锁；
