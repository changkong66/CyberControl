...
- 所有变更必须来自 PR。
- 当前 `Solo` 模式要求 0 个批准，不强制 Code Owner 审核，也不要求最后一次可审核推送
  获得独立批准；这是单人维护仓库避免自审死锁的固定策略，不是管理员绕过。
- `Team` 模式恢复 1 个批准、Code Owner 审核和最后推送批准。获得独立维护者后必须显式
  切换为 `Team`，不得手工只修改其中一个控制面。
...
force-push 和禁止删除全部生效。`MaintenanceMode` 默认值为 `Solo`；脚本必须把同一模式
同时应用到 Classic Protection 与 `main-release-governance` Ruleset，并按模式精确回读
审批数、Code Owner 审核和最后推送批准。API 回读证据写入受控验收台账。

当前参考状态：Topic 4 PR #16 已 Squash Merge，受保护 `main` 为
`190ed863c13f8f71d909b6083b929c899e4db69f`，Run `29639495363` 八项任务成功。
Python 覆盖率门禁配置为 90%；91.19% 是该 Topic 4 验收提交的观测值。

### 4.1 服务端规则固定参数

以下控制在 `Solo` 与 `Team` 模式中完全相同：
...
| Branch Ruleset | Merge methods | squash or rebase only |
...
评审参数必须作为完整模式切换，Classic Protection 与 Ruleset 不得漂移：

| Maintenance mode | Approval count | Code Owner review | Last-push approval | 使用边界 |
|---|---:|---|---|---|
| `Solo`（当前、默认） | 0 | disabled | disabled | 仅有一个可维护账号 |
| `Team` | 1 | enabled | enabled | 至少有一个独立评审人 |

`.github/CODEOWNERS` 在两种模式下都保留。Solo 模式把它作为责任归属和审计文档，不把
PR 作者无法完成的自我批准作为合并条件。
...
5. 打开 `Settings > Branches > Branch protection rules > main`，确认管理员同样受约束、
   对话解决和严格状态检查开启；评审数、CODEOWNER 审核、最后推送批准必须与当前
   `MaintenanceMode` 一致。
