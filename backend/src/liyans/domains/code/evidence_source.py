...
class PostgresCodeEvidenceSource:
    """Loads the exact immutable Candidate, C2 bundle, and Topic1 snapshot."""
    ...
    async def load(self, claim: ClaimV1) -> CodeEvidenceBundle:
        ...
        async with self._database.transaction(context=current_session_context()) as session:
            ...
            if bundle is None:
                return CodeEvidenceBundle(candidate, None, ())
            if (
                ...
                raise ValueError("code evidence bundle is not bound to the Claim")
            if not record_integrity_valid(bundle):
                raise ValueError("code evidence bundle record integrity check failed")
            ...
            if knowledge_base is None:
                raise ValueError("code evidence references an unavailable knowledge base")
            if (
                knowledge_base.knowledge_base_version_id != bundle.knowledge_base_version_id
                or not record_integrity_valid(knowledge_base)
            ):
                raise ValueError("code knowledge base binding or integrity check failed")
            ...
            if snapshot is None:
                raise ValueError("code knowledge base references an unavailable Topic1 snapshot")
            if (
                snapshot.snapshot_id != knowledge_base.graph_snapshot_id
                or snapshot.graph_version != knowledge_base.graph_snapshot_version
            ):
                raise ValueError("code Topic1 snapshot binding failed")
        ...
        if len(by_id) != len(refs):
            raise ValueError("code evidence repository returned duplicate references")
        missing = set(bundle.evidence_ref_ids) - set(by_id)
        if missing:
            raise ValueError("code evidence bundle references unavailable evidence")
        evidence = tuple(by_id[identifier] for identifier in bundle.evidence_ref_ids)
        self._validate_evidence(evidence, claim, bundle.knowledge_base_version_id)
    ...
    def _validate_candidate(candidate: CandidateV1 | None, claim: ClaimV1) -> None:
        ...
        if (
            ...
            raise ValueError("code Candidate is not bound to the Claim")
        actual = canonical_sha256(candidate.model_dump(mode="json", exclude={"candidate_sha256"}))
        if actual != candidate.candidate_sha256:
            raise ValueError("code Candidate integrity check failed")
    ...
    def _validate_evidence(
        ...
        for ref in evidence:
            if ref.tenant_id != claim.tenant_id:
                raise ValueError("code evidence crosses tenant boundaries")
            if ref.verification_id != claim.verification_id or ref.claim_id != claim.claim_id:
                raise ValueError("code evidence is not bound to the Claim")
            if ref.trace_id != claim.trace_id:
                raise ValueError("code evidence is not bound to the Trace")
            if ref.knowledge_base_version_id != knowledge_base_version_id:
                raise ValueError("code evidence is not bound to the knowledge base version")
            if not record_integrity_valid(ref):
                raise ValueError("code evidence record integrity check failed")
            if canonical_sha256(ref.excerpt) != ref.excerpt_sha256:
                raise ValueError("code evidence excerpt integrity check failed")
            if ref.evidence_ref_id in seen:
                raise ValueError("code evidence contains duplicate references")
