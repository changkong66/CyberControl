...
class ExtensionEvidenceSource(Protocol):
    async def load(self, claim: ClaimV1) -> ExtensionEvidenceBundle: ...


class PostgresExtensionEvidenceSource:
    """Loads the exact Candidate, C2 authority bundle, and Topic1 snapshot for C7."""
    ...
    async def load(self, claim: ClaimV1) -> ExtensionEvidenceBundle:
        ...
        async with self._database.transaction(context=current_session_context()) as session:
            ...
            if bundle is None:
                return ExtensionEvidenceBundle(candidate, None, ())
            if (
                ...
                raise ValueError("extension evidence bundle is not bound to the Claim")
            if not record_integrity_valid(bundle):
                raise ValueError("extension evidence bundle record integrity check failed")
            ...
            if knowledge_base is None:
                raise ValueError("extension evidence references an unavailable knowledge base")
            if (
                knowledge_base.knowledge_base_version_id != bundle.knowledge_base_version_id
                or not record_integrity_valid(knowledge_base)
            ):
                raise ValueError("extension knowledge base binding or integrity check failed")
            ...
            if snapshot is None:
                raise ValueError(
                    "extension knowledge base references an unavailable Topic1 snapshot"
                )
            if (
                snapshot.snapshot_id != knowledge_base.graph_snapshot_id
                or snapshot.graph_version != knowledge_base.graph_snapshot_version
            ):
                raise ValueError("extension Topic1 snapshot binding failed")
        ...
        if len(by_id) != len(refs):
            raise ValueError("extension evidence repository returned duplicate references")
        missing = set(bundle.evidence_ref_ids) - set(by_id)
        if missing:
            raise ValueError("extension evidence bundle references unavailable evidence")
    ...
    def _validate_candidate(candidate: CandidateV1 | None, claim: ClaimV1) -> None:
        ...
        if (
            ...
            raise ValueError("extension Candidate is not bound to the Claim")
        if canonical_sha256(candidate.model_dump(mode="json", exclude={"candidate_sha256"})) != (
            candidate.candidate_sha256
        ):
            raise ValueError("extension Candidate integrity check failed")
    ...
    def _validate_evidence(
        ...
        for ref in evidence:
            if ref.tenant_id != claim.tenant_id:
                raise ValueError("extension evidence crosses tenant boundaries")
            if ref.verification_id != claim.verification_id or ref.claim_id != claim.claim_id:
                raise ValueError("extension evidence is not bound to the Claim")
            if ref.trace_id != claim.trace_id:
                raise ValueError("extension evidence is not bound to the Trace")
            if ref.knowledge_base_version_id != knowledge_base_version_id:
                raise ValueError("extension evidence is not bound to the knowledge base version")
            if not record_integrity_valid(ref):
                raise ValueError("extension evidence record integrity check failed")
            if canonical_sha256(ref.excerpt) != ref.excerpt_sha256:
                raise ValueError("extension evidence excerpt integrity check failed")
            if ref.evidence_ref_id in seen:
                raise ValueError("extension evidence contains duplicate references")
