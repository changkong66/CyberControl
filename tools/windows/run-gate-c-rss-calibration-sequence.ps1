...
$armRunner = Join-Path $PSScriptRoot "run-gate-c-rss-calibration-arm.ps1"
...
$secretsDirectory = Join-Path $sequenceDirectory "secrets"
...
function Invoke-Arm {
    ...
    if (
        ...
        throw "$Arm arm package reference failed verification."
    }
    ...
    $eligibleForContinuation = (
        ...
        $summary.variable -eq $Variable -and
        $summary.passed -eq $true
    )
    return [ordered]@{
        path = $referencePath
        sha256 = Get-FileSha256 $referencePath
...
function Assert-ArmContinuation {
    ...
    if ($Reference.eligible_for_continuation -ne $true) {
        throw "$Arm arm is not eligible for continuation; A' is prohibited after an A or Measurement failure."
    }
}
...
Write-NewJson -Path (Join-Path $evidenceDirectory "sequence-metadata.json") -Value ([ordered]@{
    schema_version = "cybercontrol.gate-c-rss-calibration-sequence.v1"
...
try {
    ...
    $references.Measurement = Invoke-Arm `
        -Arm "Measurement" `
        -PriorArmResult ([string]$references.A.result_path)
    Assert-ArmContinuation -Arm "Measurement" -Reference $references.Measurement
    $references.APrime = Invoke-Arm `
        -Arm "APrime" `
        -PriorArmResult ([string]$references.Measurement.result_path)
    Assert-ArmContinuation -Arm "APrime" -Reference $references.APrime
    & uv run --frozen python $compareTool `
        --a ([string]$references.A.result_path) `
        --measurement ([string]$references.Measurement.result_path) `
    ...
    Write-NewJson -Path (Join-Path $evidenceDirectory "sequence-failure.json") -Value ([ordered]@{
        schema_version = "cybercontrol.gate-c-rss-calibration-sequence-failure.v1"
        process_version = $processVersion
...
Write-NewJson -Path (Join-Path $evidenceDirectory "arm-package-index.json") -Value ([ordered]@{
    schema_version = "cybercontrol.gate-c-rss-calibration-arm-package-index.v1"
    process_version = $processVersion
    sequence_id = $sequenceId
    variable = $Variable
    arms = $references
...
try {
    ...
    if ($archiveVerified) {
        ...
        if (-not $resolvedSecrets.StartsWith($resolvedSequence, [StringComparison]::OrdinalIgnoreCase)) {
            throw "Refusing to remove secrets outside the sequence directory."
        }
        ...
        if (-not $resolvedEvidence.StartsWith($resolvedSequence, [StringComparison]::OrdinalIgnoreCase)) {
            throw "Refusing to remove evidence intermediates outside the sequence directory."
        }
        ...
        Write-NewJson -Path $cleanupReceiptPath -Value ([ordered]@{
            schema_version = "cybercontrol.gate-c-rss-calibration-sequence-cleanup.v1"
            process_version = $processVersion
            sequence_id = $sequenceId
            tls_private_material_removed = (-not (Test-Path -LiteralPath $resolvedSecrets))
...
if ($null -ne $failureReason) {
    ...
    throw "$failureReason; immutable sequence evidence: $packageReferencePath"
}
Write-Output $packageReferencePath
