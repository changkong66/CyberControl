...
param(
    ...
    [string]$ContainerName = "liyans-topic3-postgres",
...
$bootstrapPassword = "liyans-bootstrap-topic3-local-only"
$databaseName = "liyans"
...
try {
    ...
    if (-not $existing) {
        ...
        Invoke-Native -Executable $docker -Arguments @(
            ...
            "--health-retries", "30",
            $postgresImage
        ) -Description "Topic 3 PostgreSQL acceptance container creation"
        ...
        if ($running -ne "true") {
            Invoke-Native -Executable $docker -Arguments @("start", $ContainerName) `
                -Description "Topic 3 PostgreSQL acceptance container start"
        }
    }
    ...
    for ($attempt = 1; $attempt -le 60; $attempt++) {
        ...
        if ($health -eq "unhealthy") {
            ...
            throw "The Topic 3 PostgreSQL container became unhealthy."
        }
    ...
    $evidencePath = Join-Path $repositoryRoot "artifacts\quality-gates\topic3-acceptance.json"
    [ordered]@{
        schema_version = "topic3.local-acceptance.v1"
        status = "LOCAL_ACCEPTED"
        generated_at_utc = [datetime]::UtcNow.ToString("o")
        branch = (& $git branch --show-current).Trim()
        commit = (& $git rev-parse HEAD).Trim()
        postgres = "16-alpine-pinned"
        coverage_redline_percent = 89
        release_quality_redline = "passed"
    } | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $evidencePath -Encoding utf8

    Write-Host "Topic 3 local acceptance passed." -ForegroundColor Green
