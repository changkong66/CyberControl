...
- Phase 1.1 提供多租户可信身份、异步事务、Outbox、全局幂等、审计哈希链、Artifact
  持久化和 SSE 回放恢复等生产底座。
- Topic 1 自动控制原理知识拓扑已 `ACCEPTED` 并冻结；Topic 2 六维画像、记忆衰退与
  自适应路径运行时已正式解锁，是当前唯一允许启动的上层业务阶段。
...
Topic 1 独立状态见
[`docs/topic1/acceptance-status.json`](docs/topic1/acceptance-status.json)。
